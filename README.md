...
В проекте используется следующие контейнеры 
- БД Postgres
...
`yarn docker:db` - запуск контейнера базы данных   
`yarn docker:dev` - запуск контейнера базы данных и dev сборки приложения (не в контейнере). Используется для разработки и hot reload изменений в коде
